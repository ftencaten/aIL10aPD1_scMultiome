...
### Quality control RNA
VlnPlot(ln, features = c("nCount_ATAC", "nCount_RNA","percent.mt"), ncol = 3,
        log = TRUE, pt.size = 0) + NoLegend()

VlnPlot(ln, features = c("nFeature_RNA", "nCount_RNA","percent.mt"), ncol = 3,
        log = TRUE, pt.size = 0) + NoLegend()

VlnPlot(ln, features = c("nFeature_RNA"), group.by = 'Sample Name',
        pt.size = 0) + NoLegend() + geom_hline(yintercept = 300)

VlnPlot(ln, features = c("nCount_RNA"), ncol = 3,
        log = TRUE, pt.size = 0) + NoLegend() + 
  geom_hline(yintercept = c(500, 25000))

VlnPlot(ln, features = c("nCount_ATAC"), ncol = 3,
        log = TRUE, pt.size = 0) + NoLegend() + 
  geom_hline(yintercept = c(1000, 70000))

VlnPlot(ln, features = c("pct_reads_in_peaks"), group.by = 'Sample Name',
        log = TRUE, pt.size = 0) + NoLegend() 

RidgePlot(ln, features = c("nFeature_RNA"), group.by = 'Sample Number') + NoLegend()
RidgePlot(ln, features = c("nCount_RNA"), group.by = 'Sample Number', log = T)  + NoLegend()
RidgePlot(ln, features = c("percent.mt"), group.by = 'Sample Number') + NoLegend()

RidgePlot(ln, features = c("nFeature_ATAC"), group.by = 'Sample Number') + NoLegend()
RidgePlot(ln, features = c("nCount_ATAC"), group.by = 'Sample Number', log = T)  + NoLegend()

### Quality control ATAC
VlnPlot(ln, features = c('TSS.enrichment', 'nucleosome_signal'), pt.size = 0)

RidgePlot(ln, features = c("TSS.enrichment"), group.by = 'Sample Number') + NoLegend()
RidgePlot(ln, features = c("nucleosome_signal"), group.by = 'Sample Number') + NoLegend()
